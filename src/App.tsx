...
import { 
  ...
  Download, Power, LayoutDashboard, GripVertical
} from 'lucide-react';
...
export default function App() {
  ...
  const [storeSettings, setStoreSettings] = useState({ isOpen: true }); // Nuevo: Modo Vacaciones
    ...
    const unsubBcv = onSnapshot(doc(db, 'settings', 'bcv'), (docSnap) => {
      if (docSnap.exists() && docSnap.data().rate) {
        ...
        const fetchBcv = async () => {
          ...
          } catch (e) { console.log("Error BCV"); }
        };
        fetchBcv();
      }
    });

    // Escuchar Estado de la Tienda (Abierta/Cerrada)
  ...
  useEffect(() => {
    ...
    // Solo descargar órdenes si es admin, preparador o motorizado
  ...
  const handleSaveBcvRate = async (newRate) => {
    ...
    try { await setDoc(doc(db, 'settings', 'bcv'), { rate: Number(newRate) }, { merge: true }); } 
    catch (error) { console.error(error); }
  };

  const handleToggleStore = async () => {
    try { await setDoc(doc(db, 'settings', 'store'), { isOpen: !storeSettings.isOpen }, { merge: true }); } 
    catch (error) { console.error(error); }
  };
    ...
    <div className="min-h-screen bg-stone-50 flex flex-col font-sans overflow-x-hidden">
      {!storeSettings.isOpen && (
        <div className="bg-red-600 text-white text-center py-2 px-4 font-bold text-sm shadow-md z-50 sticky top-0">
          ⚠️ NUESTRA TIENDA ESTÁ CERRADA TEMPORALMENTE. No estamos recibiendo nuevos pedidos en este momento. ⚠️
        </div>
      )}
      <Navbar 
        user={currentUser} 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (view === 'login') {
        ...
        // ASIGNACIÓN DE ROLES AUTOMÁTICA SEGÚN EL CORREO
        let newRole = 'client';
        const emailLower = formData.email.toLowerCase();
        if (emailLower.includes('admin')) newRole = 'admin';
        else if (emailLower.includes('motorizado')) newRole = 'motorizado';
        else if (emailLower.includes('preparador')) newRole = 'preparador';
          ...
          {user && user.role !== 'client' ? (
            <>
              <div className="text-sm text-gray-600 hidden md:block">
                Rol: <span className="font-bold text-gray-800 uppercase">{user.role}</span>
              </div>
...
// --- ADMIN COMPONENTS ---
function AdminDashboard({ products, categories, orders, systemUsers, bcvRate, currentUser, storeSettings, handleToggleStore }) {
  
  // LOGICA DE ROLES: Forzar pestaña inicial según rol
  const getInitialTab = () => {
    if (currentUser?.role === 'motorizado') return 'delivery';
    if (currentUser?.role === 'preparador') return 'kanban';
    return 'orders'; // admin default
  };
  ...
  return (
    <div className="animate-fade-in flex flex-col md:flex-row gap-6">
      <div className="w-full md:w-64 shrink-0 print:hidden">
        <div className="bg-white rounded-3xl shadow-sm border border-stone-200 p-5 sticky top-24">
          <div className="flex items-center gap-3 mb-6 px-2">
            ...
            <div>
              <h3 className="font-bold text-gray-800 leading-tight">Panel Interno</h3>
              <p className="text-[10px] text-stone-500 uppercase tracking-widest">{currentUser?.role}</p>
            </div>
          </div>
          
          <nav className="space-y-1.5">
            {isAdmin && (
              <button onClick={() => setActiveTab('kpis')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl transition-all font-medium text-sm ${activeTab === 'kpis' ? 'bg-stone-900 text-white shadow-md' : 'text-stone-600 hover:bg-stone-100'}`}><TrendingUp className="w-5 h-5" /> Dashboard & Reportes</button>
            )}
            ...
            {isAdmin && (
              <button onClick={() => setActiveTab('orders')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl transition-all font-medium text-sm ${activeTab === 'orders' ? 'bg-stone-900 text-white shadow-md' : 'text-stone-600 hover:bg-stone-100'}`}>
                <ShoppingBag className="w-5 h-5" /> Tabla de Pedidos 
            ...
            {isPreparador && (
              <button onClick={() => setActiveTab('kanban')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl transition-all font-medium text-sm ${activeTab === 'kanban' ? 'bg-stone-900 text-white shadow-md' : 'text-stone-600 hover:bg-stone-100'}`}><LayoutDashboard className="w-5 h-5" /> Producción (Kanban)</button>
            )}
      ...
      <div className="flex-1 min-w-0">
        {activeTab === 'kpis' && isAdmin && <AdminKPIs orders={orders} products={products} bcvRate={bcvRate} />}
...
// --- NUEVO: TABLERO DE PRODUCCIÓN (KANBAN) ---
function AdminKanban({ orders }) {
  // Filtrar y mapear órdenes
  ...
  const cols = [
    ...
    { id: 'Listos', title: 'Listos (Para Entregar)', color: 'border-green-500 bg-green-50/50', icon: <CheckCircle className="w-5 h-5 text-green-500"/> }
  ];
  ...
  const handleDrop = async (e, newStatus) => {
    ...
    // Si la columna es 'Listos', el status real que guardamos puede ser 'Completado' o 'Pagado' (asumimos Completado visualmente, pero mantendremos la lógica de negocio).
    // Para no romper la contabilidad, usaremos un mapeo simple:
    let finalStatus = newStatus;
    if (newStatus === 'Listos') finalStatus = 'Completado'; 
  ...
  return (
    <div className="space-y-6 animate-fade-in w-full h-full">
      <div>
        <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2"><LayoutDashboard className="w-6 h-6 text-purple-600"/> Tablero de Producción</h2>
        <p className="text-stone-500">Arrastra las tarjetas para organizar el flujo de trabajo en la cocina.</p>
      </div>

      <div className="flex flex-col xl:flex-row gap-6 w-full min-h-[600px] pb-10">
        {cols.map(col => {
          // Filtrar órdenes por columna. Tratamos 'Pagado' y 'Abonado' como 'Pendiente' en el kanban si no están en preparación.
          const colOrders = validOrders.filter(o => {
            if (col.id === 'Pendiente') return ['Pendiente', 'Abonado', 'Pagado'].includes(o.status);
            if (col.id === 'En Preparación') return o.status === 'En Preparación';
            if (col.id === 'Listos') return o.status === 'Completado';
            return false;
          });
          ...
          return (
            <div 
              ...
              className={`flex-1 flex flex-col bg-stone-100 rounded-3xl border-t-4 shadow-sm p-4 ${col.color}`}
            >
              <div className="flex justify-between items-center mb-4 pb-2 border-b border-stone-200">
                <h3 className="font-bold text-gray-800 flex items-center gap-2">{col.icon} {col.title}</h3>
                <span className="bg-white text-stone-600 font-bold px-2 py-0.5 rounded-full text-xs shadow-sm">{colOrders.length}</span>
              </div>
              ...
              <div className="flex flex-col gap-3 flex-grow">
                {colOrders.map(order => (
                  <div 
                    ...
                    className="bg-white p-4 rounded-2xl shadow-sm border border-stone-200 cursor-grab active:cursor-grabbing hover:shadow-md transition-shadow relative group"
                  >
                    <GripVertical className="absolute right-2 top-4 text-stone-300 opacity-0 group-hover:opacity-100 transition-opacity w-5 h-5"/>
                    <div className="mb-2">
                      <span className="font-black text-gray-900 block">{order.displayId}</span>
                      <span className="text-xs text-stone-500 font-medium">Para: {order.recipientName || order.customerName}</span>
                    </div>
                    
                    <div className="bg-stone-50 p-2 rounded-lg border border-stone-100 mb-3 space-y-1">
                      <p className="text-[10px] font-bold text-stone-400 uppercase">Productos:</p>
                      {order.items.map((item, idx) => (
                        <p key={idx} className="text-xs font-bold text-gray-700 leading-tight">
                          <span className="text-red-500 mr-1">{item.quantity}x</span> {item.name}
                        </p>
                      ))}
                    </div>
                    ...
                    <div className="flex justify-between items-end">
                      <div>
                        {order.deliveryMethod === 'pickup' ? (
                          <span className="bg-blue-100 text-blue-700 text-[10px] font-bold px-2 py-1 rounded flex items-center gap-1 w-max"><MapPin className="w-3 h-3"/> Retiro Tienda</span>
                        ) : (
                          <span className="bg-stone-100 text-stone-600 text-[10px] font-bold px-2 py-1 rounded flex items-center gap-1 w-max"><Truck className="w-3 h-3"/> Delivery</span>
                        )}
                      </div>
                      <span className="text-[10px] font-bold text-stone-400">{order.deliveryDate || 'Hoy'}</span>
                    </div>
                    
                    {order.dedication && (
                       <div className="mt-3 pt-2 border-t border-dashed border-stone-200 text-[11px] italic text-stone-600 bg-yellow-50/50 p-2 rounded">
                         "{order.dedication}"
                       </div>
                    )}
                  </div>
                ...
                {colOrders.length === 0 && (
                  <div className="h-full flex items-center justify-center border-2 border-dashed border-stone-300 rounded-2xl opacity-50 p-6 text-center text-sm font-medium text-stone-500">
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-stone-500">Selecciona los pedidos y arma la ruta específica para el delivery.</p>
      </div>
...
// --- MÓDULO REDISEÑADO: EXPORTAR A CSV Y SEMÁFORO STOCK ---
function AdminKPIs({ orders, products, bcvRate }) {
  const now = new Date();
  ...
  const criticalStock = products.filter(p => p.stock !== '' && p.stock !== undefined && Number(p.stock) <= 3);

  // NUEVO: Exportar a CSV
  const exportToCSV = () => {
    const headers = ['ID Pedido', 'Fecha Sistema', 'Cliente Emisor', 'Tlf Emisor', 'Destinatario', 'Método Entrega', 'Total USD', 'Estado'];
    const rows = validOrders.map(o => {
      const dateStr = o.date ? new Date(o.date).toLocaleDateString() : '';
      const sender = `"${(o.senderName || o.customerName || '').replace(/"/g, '""')}"`;
      const recipient = `"${(o.recipientName || '').replace(/"/g, '""')}"`;
      return [o.displayId, dateStr, sender, o.senderPhone || o.phone, recipient, o.deliveryMethod, o.totalUSD, o.status];
    });
    ...
    link.setAttribute("download", `ventas_decomer_${todayStr.replace(/\//g, '-')}.csv`);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button onClick={exportToCSV} className="bg-stone-800 hover:bg-black text-white px-5 py-2.5 rounded-xl flex items-center justify-center gap-2 font-bold transition-all shadow-lg text-sm w-full sm:w-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
        <div className="bg-stone-100 p-6 rounded-3xl border border-stone-200 flex flex-col justify-center">
          <div>
          ...
          <div className="mt-2">
            <p className="text-3xl font-black text-gray-900">${totalHistóricoUSD.toFixed(2)}</p>
          </div>
        </div>

        {/* SEMÁFORO DE INVENTARIO */}
        <div className={`p-6 rounded-3xl border ${criticalStock.length > 0 ? 'bg-red-50 border-red-200' : 'bg-green-50 border-green-200'}`}>
          <h4 className={`font-bold flex items-center gap-2 mb-4 ${criticalStock.length > 0 ? 'text-red-800' : 'text-green-800'}`}>
            <ShieldAlert className="w-5 h-5"/> Alertas de Inventario
          </h4>
          {criticalStock.length > 0 ? (
            <div className="space-y-2 max-h-32 overflow-y-auto pr-2">
              {criticalStock.map(p => (
                <div key={p.id} className="flex justify-between items-center bg-white p-2.5 rounded-xl border border-red-100 shadow-sm">
                  <span className="text-sm font-bold text-gray-800 truncate pr-2">{p.name}</span>
                  <span className="bg-red-100 text-red-700 text-xs font-black px-2 py-1 rounded shrink-0">{Number(p.stock) === 0 ? 'AGOTADO' : `Quedan ${p.stock}`}</span>
                </div>
              ))}
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center h-24 opacity-60">
              ...
              <p className="text-xs font-bold text-green-800">Todo tu inventario está saludable.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="p-8 bg-red-50 border border-red-200 rounded-3xl text-center shadow-inner">
          ...
          <p className="text-red-600 font-medium max-w-md mx-auto mb-4">
            Un problema impidió cargar la lista de clientes.
          </p>
...
function AdminProducts({ products, categories }) {
  ...
  const [searchTerm, setSearchTerm] = useState('');
  const [currentProduct, setCurrentProduct] = useState({ id: '', name: '', price: '', image: '', description: '', categoryId: '', badge: '', isExtra: false, emoji: '', stock: '' });
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-stone-500">Gestiona tu oferta de arreglos y stock (Semáforo)</p>
        </div>
        <button onClick={() => { setCurrentProduct({ id: '', name: '', price: '', image: '', description: '', categoryId: categories[0]?.id || '', badge: '', isExtra: false, emoji: '', stock: '' }); setIsEditing(true); }}
        ...
        <div className="bg-white p-6 rounded-3xl shadow-xl border border-stone-200 relative animate-scale-in">
          ...
          <form onSubmit={handleSaveProduct} className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="flex gap-3">
              ...
              <div className="w-24">
                <label className="block text-xs font-bold text-gray-700 mb-1 uppercase" title="Dejar vacío si es ilimitado">Stock</label>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-stone-50 text-stone-500 text-xs uppercase tracking-wider border-b border-stone-200">
              ...
              <th className="p-4 font-bold text-center">Tipo / Stock</th>
          ...
          <tbody className="divide-y divide-stone-100">
            {filteredProducts.map((product) => {
              const isLowStock = product.stock !== '' && product.stock !== undefined && Number(product.stock) <= 3;
              return (
              <tr key={product.id} className="hover:bg-stone-50/50 transition-colors">
                ...
                <td className="p-4 text-center">
                   {product.isExtra ? <span className="text-xs font-bold text-blue-600 bg-blue-50 px-3 py-1 rounded-full border border-blue-100 block w-max mx-auto mb-1">Extra / Upsell</span> : <span className="text-xs font-bold text-stone-500 bg-stone-100 px-3 py-1 rounded-full border border-stone-200 block w-max mx-auto mb-1">Catálogo</span>}
                   {product.stock !== '' && product.stock !== undefined ? (
                     <span className={`text-[10px] font-black px-2 py-0.5 rounded ${isLowStock ? 'bg-red-100 text-red-700 border border-red-200' : 'text-stone-500'}`}>Stock: {product.stock}</span>
                   ) : (
                     <span className="text-[10px] text-stone-400">Ilimitado</span>
                   )}
                </td>
...
function ClientStorefront({ products, categories, cart, setCart, user, bcvRate, searchQuery, storeIsOpen }) {
  ...
  const addToCart = (product) => {
    if (!storeIsOpen) return alert("La tienda está cerrada temporalmente. ¡Vuelve pronto!");
  ...
  const handleCheckout = async (e) => {
    e.preventDefault();
    if (!storeIsOpen) return alert("La tienda ha cerrado y no podemos procesar este pedido.");
    ...
    // Descontar inventario al confirmar compra
              ...
              <div className="p-4 sm:p-5 flex flex-col flex-grow">
                ...
                <div className="flex flex-wrap items-center justify-between mt-auto pt-4 border-t border-stone-100 gap-2">
                  ...
                  <div className="flex gap-1.5 sm:gap-2 shrink-0 ml-auto">
                    <button disabled={!storeIsOpen || isOutOfStock} onClick={() => handleQuickBuy(product)} title="Comprar Ahora" className="bg-gray-100 text-gray-600 hover:bg-stone-800 hover:text-white disabled:bg-stone-100 disabled:text-stone-400 p-2.5 rounded-xl transition-colors shrink-0"><Zap className="w-4 h-4 sm:w-5 sm:h-5 fill-current" /></button>
                    <button disabled={!storeIsOpen || isOutOfStock} onClick={() => addToCart(product)} title="Añadir al carrito" className="bg-red-50 text-red-600 hover:bg-red-600 hover:text-white disabled:bg-red-50 disabled:text-red-300 p-2.5 rounded-xl transition-colors shrink-0"><Plus className="w-4 h-4 sm:w-5 sm:h-5 font-bold" /></button>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-stone-100 sticky top-24 overflow-hidden flex flex-col max-h-[calc(100vh-8rem)]">
              ...
              <div className="mt-8">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {extraProducts.map((extra) => {
                    const isExtraOut = extra.stock !== '' && extra.stock !== undefined && Number(extra.stock) <= 0;
                    return (
                    <button disabled={!storeIsOpen || isExtraOut} key={extra.id} onClick={() => addToCart(extra)} className="bg-white border border-stone-200 hover:border-pink-300 disabled:opacity-50 disabled:hover:border-stone-200 p-2 rounded-xl flex items-center gap-2 text-left transition-all hover:shadow-sm group">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px] text-red-500 font-bold">{isExtraOut ? 'Agotado' : `+$${Number(extra.price).toFixed(2)}`}</p>
                      </div>
                    </button>
                    );
                  })}
          ...
          {cart.length > 0 && (
            <div className="p-6 bg-white border-t border-stone-100">
              ...
              <button disabled={!storeIsOpen} onClick={() => setCheckoutStep(true)} className="w-full bg-[#25D366] hover:bg-[#1ebd5a] disabled:bg-stone-400 text-white font-bold py-3.5 rounded-2xl transition-all shadow-lg flex items-center justify-center gap-2">
                {storeIsOpen ? 'Ir a Pagar' : 'Tienda Cerrada'}
              </button>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 bg-stone-900/80 backdrop-blur-sm flex items-center justify-center p-4 z-[60] animate-fade-in">
          <div className="bg-white rounded-3xl w-full max-w-4xl overflow-hidden flex flex-col md:flex-row shadow-2xl relative">
            ...
            <div className="w-full md:w-1/2 p-8 md:p-10 flex flex-col justify-center bg-white">
              ...
              <div className="flex gap-3">
                <button disabled={!storeIsOpen || (previewProduct.stock !== '' && previewProduct.stock !== undefined && Number(previewProduct.stock) <= 0)} onClick={() => addToCart(previewProduct)} className="flex-1 bg-red-600 hover:bg-red-700 disabled:bg-stone-300 disabled:cursor-not-allowed text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 text-lg shadow-xl shadow-red-200 transition-transform hover:-translate-y-1">
                  <ShoppingCart className="w-5 h-5" /> {(previewProduct.stock !== '' && previewProduct.stock !== undefined && Number(previewProduct.stock) <= 0) ? 'Agotado' : 'Agregar al Pedido'}
                </button>
              </div>
            </div>
          </div>
